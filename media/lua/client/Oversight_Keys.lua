...
function Oversight.getPrevPl(currentPl)
...
function Oversight.setSpectateOffset(key)
    ...
    if not Oversight.isSpectating(pl) then return key end
    ...
    elseif key == selectPrev then
        local currentTargetUser = Oversight.getSpectateTargUser(pl)
        if currentTargetUser then
            local prevPl = Oversight.getPrevPl(currentTargetUser)
            if prevPl then
                Oversight.setSpectate(prevPl)
            end
        end
    elseif key == selectNext then
        local currentTargetUser = Oversight.getSpectateTargUser(pl)
        if currentTargetUser then
            local currentTargetUser = currentTarget:getUsername()
            local nextPl = Oversight.getNextPl(currentTargetUser, true)
            if nextPl then
                Oversight.setSpectate(nextPl)
            end
        end
    end
